...
{
	float maxValue = 0.0;
	for (NSUInteger i = visibleRange.location; i < visibleRange.location + visibleRange.length; i++) {
...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView
{
	...
	for (NSUInteger i=visibleRange.location; i<visibleRange.location+visibleRange.length; i++) {
		StackedBarView *barView = [visibleBarViews objectForKey:[NSNumber numberWithUnsignedInteger:i]];
		...
		if (!barView) {
			...
			[visibleBarViews setObject:barView forKey:[NSNumber numberWithUnsignedInteger:i]];
		}
	}
}
